...
def test_schema_evolution_pass(valid_df):
    result = _check_schema_evolution(valid_df)
    assert result.status == "pass"

def test_schema_evolution_warn_new_column(valid_df):
    df = valid_df.copy()
    df["vendor"] = "Nokia"
...
def test_schema_evolution_fail_on_type_change(valid_df):
    df = valid_df.copy()
    df["signal_strength"] = df["signal_strength"].astype(str)  # float → object
    result = _check_schema_evolution(df)
    assert result.status == "fail"
    assert "signal_strength" in result.message
