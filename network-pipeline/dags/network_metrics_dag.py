...
─── DAG flow ────────────────────────────────────────────────────────────────────
  ...
  ingest_bronze                          mark_skipped
      ...
      │  Schema check (type change → abort)   │
      │  Evolve + write bronze Iceberg table  │
      │                                  end_pipeline
  run_dq_checks                               ▲
      │  7 checks: not_empty, schema_columns, │
      │   null_check, signal_range,           │
      │   data_volume_positive,               │
      │  timestamp_freshness, duplicate_rows  │
...
def _run_dq_checks(**context):
    ...
    from silver_processing import _write_dq_log
    ...
    for d in dates:
        ...
        _write_dq_log(results, d, run_id)
