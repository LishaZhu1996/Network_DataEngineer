...
def write_iceberg(
    ...
    # Evolve Iceberg schema for any new columns in the incoming data
    _evolve_schema(ice_table, pa_table)
...
def _evolve_schema(ice_table, pa_table: pa.Table) -> None:
    """Add any new columns from pa_table into the Iceberg table schema."""
    from pyiceberg.io.pyarrow import pyarrow_to_schema

    existing_names = {f.name for f in ice_table.schema().fields}
    incoming_schema = pyarrow_to_schema(pa_table.schema)
    new_fields = [f for f in incoming_schema.fields if f.name not in existing_names]
    ...
    logger.info(
        "Evolved schema of %s — added columns: %s",
        ice_table.name(),
