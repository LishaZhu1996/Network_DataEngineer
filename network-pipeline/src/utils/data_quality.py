...
def _check_nulls(df: pd.DataFrame) -> DQResult:
    ...
    # Treat nulls in key columns as fail, others as warn
    key_nulls = null_counts[["tower_id", "region", "timestamp"]].sum() if all(c in df.columns for c in ["tower_id", "region", "timestamp"]) else 1
    status = "fail" if key_nulls > 0 else "warn"
    return DQResult("null_check", status, f"Nulls found — {detail}", failed_rows=total_nulls)
