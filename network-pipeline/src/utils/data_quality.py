...
# Tracks the last-seen schema so evolution checks can compare against it.
# In prod this would come from a schema registry or the Iceberg table itself.
_KNOWN_COLUMNS: set = set(REQUIRED_COLUMNS)
...
def _check_schema_evolution(df: pd.DataFrame) -> DQResult:
    """Detect schema changes relative to the known column set.

    - New columns        → warn  (safe to add, Iceberg will auto-evolve)
    - Dropped columns    → warn  (existing Iceberg rows will show NULL)
    - Column type change → fail  (could corrupt existing data)
    """
    incoming = set(df.columns)
    new_cols     = incoming - _KNOWN_COLUMNS
    dropped_cols = _KNOWN_COLUMNS - incoming

    # Type change check: for columns present in both, compare pandas dtype against expected
    type_mismatches = []
    for col, expected_kind in EXPECTED_DTYPES.items():
        if col not in df.columns:
            continue
        actual = df[col].dtype
        # expected_kind is "int", "float", or "object" — check the dtype kind prefix
        ...
        elif expected_kind == "object" and not pd.api.types.is_object_dtype(actual):
            type_mismatches.append(f"{col}: expected string/object, got {actual}")

    if type_mismatches:
        return DQResult(
            "schema_evolution", "fail",
            f"Column type mismatch detected: {type_mismatches}. Pipeline aborted to prevent data corruption.",
        )
    if new_cols:
        return DQResult(
            "schema_evolution", "warn",
            f"New columns detected: {new_cols}. Iceberg schema will be evolved automatically.",
        )
    if dropped_cols:
        return DQResult(
            "schema_evolution", "warn",
            f"Columns no longer in source: {dropped_cols}. Existing Iceberg rows will show NULL for these columns.",
        )
    return DQResult("schema_evolution", "pass", "No schema changes detected.")
